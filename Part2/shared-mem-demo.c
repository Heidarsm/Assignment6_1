...
// when enter is pressed inside one of the processes, the process begins reading the shared memory, then writing into the shared memory "Hello from process ###".
// That is why when the program is ran for the first time the process gets and empty string becouse no process has written into the shared memory and initially
// it is empty. THen the 2nd process that writes into the shared memory gets the string from the 1st process that ran the commands, and overwrites the previous data.